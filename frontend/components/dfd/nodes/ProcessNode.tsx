...
function ProcessNodeInner(props: NodeProps<Node<DfdNodeData>>) {
  ...
  return (
    <div
      className={`rounded-2xl border-2 bg-white px-3 py-2 shadow-sm min-w-[160px] max-w-[220px] font-sans text-sm ${border} ${selected ? 'ring-2 ring-blue-400 ring-offset-2' : ''} ${data.dimmed ? 'opacity-25' : ''}`}
      tabIndex={0}
    ...
    >
      ...
      <div className="text-[10px] uppercase tracking-wide text-slate-500 mt-0.5">Process</div>
      {stats && stats.count > 0 && (
