...
- **DeepInfra threat-modeling jobs no longer fail intermittently with "No threat modeling JSON report was generated by the agent."** `appsec-agent` 4.0.1 now sends an explicit `max_tokens` budget on DeepInfra requests (clamped to the model's context window) instead of relying on DeepInfra's conservative implicit output cap (~32-36K tokens). A large STRIDE report sits right at that ceiling, so runs needing slightly more completion tokens were clipped mid-JSON (`finish_reason: "length"`) and silently dropped — the job finished with a billed cost but wrote no report. The DeepInfra provider also surfaces the drop reason (truncation / schema failure / max-turns) instead of swallowing it.

### Changed

- **Bumped `appsec-agent` to `^4.0.1`** (from `^4.0.0`) in the root and backend packages.
- **Root, backend, and frontend package versions bumped to `3.0.1`.**
